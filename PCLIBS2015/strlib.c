...
string SubString(string s, int p1, int p2)
{
	...
	strncpy_s(result, len + 1, s + p1, len);
	result[len] = '\0';
	return (result);
}
...
int StringToInteger(string s)
{
	...
	if (sscanf_s(s, " %d %c", &result, &dummy, 1) != 1) {
...
double StringToReal(string s)
{
	...
	if (sscanf_s(s, " %lg %c", &result, &dummy, 1) != 1) {
